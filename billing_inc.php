...
$t_docnote_stats_from_def = $t_date_submitted;
...
$t_docnote_stats_from_d = gpc_get_int( 'start_day', $t_docnote_stats_from_def_d );
$t_docnote_stats_from_m = gpc_get_int( 'start_month', $t_docnote_stats_from_def_m );
$t_docnote_stats_from_y = gpc_get_int( 'start_year', $t_docnote_stats_from_def_y );

$t_docnote_stats_to_def = $t_today;
...
$t_docnote_stats_to_d = gpc_get_int( 'end_day', $t_docnote_stats_to_def_d );
$t_docnote_stats_to_m = gpc_get_int( 'end_month', $t_docnote_stats_to_def_m );
$t_docnote_stats_to_y = gpc_get_int( 'end_year', $t_docnote_stats_to_def_y );
...
$f_docnote_cost = floatval( gpc_get_string( 'docnote_cost', '' ) );
...
<form method="post" action="">
    ...
    ?>
        ...
        <input type="text" name="docnote_cost" class="input-sm" value="<?php echo $f_docnote_cost ?>" />
    <?php
        }
    ?>
    </div>
...
<?php
	if( !is_blank( $f_get_docnote_stats_button ) ) {
		...
		$t_docnote_stats = docnote_stats_get_project_array( $f_project_id, $t_from, $t_to, $f_docnote_cost );
		...
		array_multisort( $t_sort_bug, SORT_NUMERIC, $t_sort_name, $t_docnote_stats );
		unset( $t_sort_bug, $t_sort_name );

		if( is_blank( $f_docnote_cost ) || ( (double)$f_docnote_cost == 0 ) ) {
			$t_cost_col = false;
		}
...
<?php	if( $t_cost_col ) { ?>
		<td class="small-caption pull-right">
			<?php echo string_attribute( number_format( $t_sum_in_minutes * $f_docnote_cost / 60, 2 ) ); ?>
		</td>
<?php 	} ?>
	</tr>
</table>
...
<?php		if( $t_cost_col ) { ?>
		<td class="small-caption pull-right">
			<?php echo string_attribute( number_format( $t_total_time * $f_docnote_cost / 60, 2 ) ); ?>
		</td>
<?php		} ?>
	</tr>
<?php	} ?>
