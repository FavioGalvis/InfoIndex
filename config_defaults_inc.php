 ...
 * read/write access to the InfoIndex database. The default user name is "root".
...
$g_db_password			= '';
/**
 * Name of database that contains InfoIndex tables.
 * The default database name is "infoindex".
 * @global string $g_database_name
 */
$g_database_name		= 'infoindex';
 ...
 * "Driver={SQL Server Native Client 10.0};SERVER=.\sqlexpress;DATABASE=infoindex;UID=root;PWD=password;"
...
/**
 * Path to root InfoIndex folder.  Requires trailing / or \
 ...
 * If INFOINDEX_CONFIG_FOLDER environment variable is set, it will be used.
 ...
 */
$t_local_config = getenv( 'INFOINDEX_CONFIG_FOLDER' );
 ...
 * documentation at http://www.informatica-tr.com. No check is performed on URLs.
...
/**
 * Master salt value used for cryptographic hashing throughout InfoIndex. This
 ...
 * your InfoIndex installation. Failure to set this configuration option
 * correctly could lead to your InfoIndex installation being compromised. Ensure
 ...
 *
 *      'uploader': the reporter of the document
 *       'handler': the handler of the document
 *       'monitor': users who are monitoring a document
 *      'docnotes': users who have added a docnote to the document
 ...
 *
 *             'new': a new document has been added
 *           'owner': a document has been assigned to a new owner
 *      'reassigned': a document has been reopened
 *         'deleted': a document has been deleted
 *         'updated': a document has been updated
 *         'docnote': a docnote has been added to a document
 *         'sponsor': sponsorship has changed on this document
 *        'relation': a relationship has changed on this document
 *         'monitor': a document is monitored.
 *        '<status>': eg: 'indexed', 'denied', 'feedback', 'acknowledged', etc.
 *                     this list corresponds to $g_status_enum_string
 *
 * If you wanted to have all editors get notified of new documents you might add
 * the following lines to your config file:
 *
 * $g_notify_flags['new']['threshold_min'] = EDITOR;
 * $g_notify_flags['new']['threshold_max'] = EDITOR;
 *
 * You might want to do something similar so all managers are notified when a
 * document is denied.  If you did not want uploaders to be notified when a 
 * document is closed (only when it is indexed) you would use:
 *
 * $g_notify_flags['denied']['uploader'] = OFF;
 *
...
/**
 * We don't need to send these notifications on new documents
...
/**
 * String appended to the InfoIndex version when displayed to the user
 ...
 */
$g_logo_image = 'images/infoindex_logo.png';
 ...
 * OFF: Shows priority as icon in view all documents page
 * ON:  Shows priority as text in view all documents page
 ...
 */
$g_show_document_project_links = ON;
...
/**
 * Position of action buttons when viewing documents.
...
/**
 * show project versions in create, view and update screens
